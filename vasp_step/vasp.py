...
class VASP(seamm.Node):
    ...
    def description_text(self, P=None):
        ...
        while node is not None:
            try:
                ...
                print(
                    f"Error describing vasp flowchart: {e} in {node}"
                )
                logger.critical(
                    f"Error describing vasp flowchart: {e} in {node}"
                )
                ...
                print(
                    "Unexpected error describing vasp flowchart: {} in {}"
    ...
    def run(self):
        ...
        result = local.run(
            cmd=["VASP", "-in", "molssi.dat"],
            files=files,
            return_files=[]
        )  # yapf: disable

        if result is None:
            logger.error("There was an error running VASP")
            return None
